...
export class MFilter {
  ...
  constructor(param:any){
      this.filter = {
        realEstateHouseDirection : param[filterKey.realEstateHouseDirection.idQuery] || '',
      ...
      this.filterClause = {
        startLandRealitySquare: param[filterKey.realEstateLandRealitySquare.idQuery]?.split('-')[0] || 0,
        endLandRealitySquare : param[filterKey.realEstateLandRealitySquare.idQuery]?.split('-')[1] || 0,
        startValueSalePrice : param[filterKey.realEstateValueSalePrice.idQuery]?.split('-')[0] * 1000000 || 0,
        endValueSalePrice : param[filterKey.realEstateValueSalePrice.idQuery]?.split('-')[1] * 1000000 || 0
      }
