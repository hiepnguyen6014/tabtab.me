...
export const FilterValuationProvider = props => {
    ...
    function handleFetchData() {
      ...
      Promise.all([
        ...
        if (res[1].length > 0 && res[0].length > 0) {
          if (!_.isEmpty(query)) {
            currentValueSelected.areaProvince = findValueSelected(
              Number(query[filterKey.areaProvinceId.idQuery]),
              'areaDataId',
              res[0],
            );
            currentValueSelected.areaDistrict = findValueSelected(
              Number(query[filterKey.areaDistrictId.idQuery]),
              'areaDataId',
              res[1],
            );
            currentValueSelected.realEstateCategory = findValueSelected(
              Number(query[filterKey.realEstateCategoryId.idQuery]),
              'realEstateCategoryId',
              res[2],
            );
            formSuggestPrice.setFieldsValue({
              areaProvince:
                currentValueSelected?.areaProvince?.areaDataId ||
                res[0].areaDataId,
              areaDistrict:
                currentValueSelected?.areaDistrict?.areaDataId ||
                res[1].areaDataId,
              realEstateCategory:
                currentValueSelected?.realEstateCategory
                  ?.realEstateCategoryId || res[2].realEstateCategoryId,
            });
  ...
  const onChangeValue = async (
    ...
    formSuggestPrice.setFieldsValue({
      ...
      realEstateCategory:
        newSelectedData.realEstateCategory.realEstateCategoryId,
    });
  ...
  const onSubmit = () => {
    ...
    const queryBuilder = `?${[filterKey.areaProvinceId.idQuery]}=${
      queryValue[filterKey.areaProvinceId.field]
    }&${[filterKey.areaDistrictId.idQuery]}=${
      queryValue[filterKey.areaDistrictId.field]
    }&${[filterKey.realEstateCategoryId.idQuery]}=${
      queryValue[filterKey.realEstateCategoryId.field]
    }`;
    if (window) {
