...
export const UserContextProvider = props => {
  ...
  useEffect(() => {
    ...
    const infoUser = getInfoUser(
      reactLocalStorage.getObject(Config.INFO_USER_KEY, {}).token,
    );
  ...
  useEffect(() => {
    ...
    if (Object.keys(infoUser).length) {
      ...
      if (
        router.query?.type == POST_TYPE_PARAMS.POST_LOCATION.params ||
        isHomePage
      ) {
        getListLocation(router.query?.page?.toString(), infoUser?.token);
      }
      if (
        router.query?.type == POST_TYPE_PARAMS.POST_SAVED.params ||
        isHomePage
      ) {
