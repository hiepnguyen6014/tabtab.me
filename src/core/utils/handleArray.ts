...
export const getPostTypeId = (pathname: string | number) => {
  ...
  for (let i = 0; i < arrayRoutes.length; i++) {
    ...
    if (typeof pathname == 'number') {
      // parameter is post type id [number]
      return {
        id: pathname,
...
export const getTitleValuationChart = (
  filter: any,
  listType?: any[],
  listDistrict?: any[],
) => {
  ...
  const getName = listType.filter(
    item =>
      item[filterKey.realEstateCategoryId.id] ==
      filter.filter[filterKey.realEstateCategoryId.id],
  )[0];
  const getNameType = getName
    ? getName[filterKey.realEstateCategoryId.name]
    : '';
  const getDistrict = listDistrict.filter(
    item => item[filterKey.areaDistrictId.id] == filter.filter?.areaDistrictId,
  )[0];
  const getDistrictName = getDistrict
    ? ', ' + 'Quan' + ' ' + getDistrict[filterKey.areaDistrictId.name]
    : '';
