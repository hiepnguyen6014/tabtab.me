/* eslint-disable react-hooks/exhaustive-deps */
import React, { useEffect, useState } from 'react';
import { Image, Typography } from 'antd';
import { DefaultContentNoGrid, DefaultWrapperWithBg } from '../../../layout/DefaultLayout/DefaultComponent.style';
import { TitlePage } from '../../../layout/TitlePage/index';
import { MNewsPost } from '@models/MNewsPost';
import { ROUTES } from '@constants';
import CategoryLayout from '../../../layout/LayoutCategory';
import { OtherNews } from './otherNews';
import { NoPost } from '@root/src/components/layout/DefaultLayout/LayoutDetail';
import { parseHtmlText } from '@utils';

export default function HotNewsDetail(props: { post: MNewsPost;others:MNewsPost[], t: any, title?:string }) {
  const { post, t: translate, others } = props;
  const arrHref=[
    {
      name: translate(ROUTES.HOTNEWS_PAGE.name),
      href: ROUTES.HOTNEWS_PAGE.href,
    },
    {
      name: translate(ROUTES.DETAIL.name),
      href: ROUTES.DETAIL.href,
    },
  ]

  const description = post.description.join('')
  useEffect(()=>{ 
    const dom = document.getElementById("news_content")
    if(description && dom) dom.innerHTML = description;
})

  return (
    <CategoryLayout t={translate} fullWidth>
      <DefaultWrapperWithBg/>
      {
        post.newsId ? <>
          <DefaultWrapperWithBg>
              <DefaultContentNoGrid>
                <TitlePage t={translate} arrayHref={arrHref} title={post.newsTitle}/>
              </DefaultContentNoGrid>
            </DefaultWrapperWithBg>
          <DefaultWrapperWithBg>
            <DefaultContentNoGrid>
              <Image
                alt=""
                src={post.introduceImage}
                height="100%"
                width="100%"
                style={{ maxHeight: '700px' }}
                preview={false}
                fallback="data:image/webp;base64,"
              />
              <br/>
              <br/>
              <section>
                {/* { post.description.map((line,index)=>
                  <p key={index}>{line}</p>  
                )} */}
                <div id="news_content"/>
              </section>
            </DefaultContentNoGrid>
          </DefaultWrapperWithBg>        
        </> : <NoPost translate={translate}/>
      }

      <OtherNews t={translate} otherNews={others}/>

    </CategoryLayout>
  );
}
