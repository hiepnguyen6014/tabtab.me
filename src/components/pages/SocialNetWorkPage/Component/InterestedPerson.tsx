...
import { VectorDollarS, VectorNextButton, VectorPrevButton } from '@root/public/icons';
        ...
        {GROUPPERSONLIST.map((item, key) => {
          return (
            <ColWrap span={7} key={key}>
                ...
                <Typography className='interested-person-name'>{item.name}</Typography>
                <Flex style={{justifyContent:"center"}}>
                  <Typography className='interested-person-text'>{item.role}</Typography>
                  <Typography className='interested-person-text'>|</Typography>
                  <Typography className='interested-person-text'>{item.company}</Typography>
                </Flex>
                <Flex style={{justifyContent:"center"}}>
