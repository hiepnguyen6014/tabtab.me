...
import { IconChartTabTab, IconCoinTabTab, Location, IconEmailTabTab, IconFBTabTab, VectorHeart, IconIN, IconInstagramTabTab, IconKey, IconMessage, IconMessTabTab, IconMortage, IconPhoneCall, IconPhoneTabTab, IconReview, IconSocial, IconStartRating, IconTiktok } from 'public/icons'
import { useState } from "react";
import { WidgetAction, WidgetHeader, WidgetWrap, ButtonCall, PostWrapper, ButtonChat } from "../FindPersonDetails.style";
...
export default function Person (props: { post: MPostDetail, t: any }) {
    ...
    return (
        <>
            <WidgetWrap>
                <div style={{ display: 'flex', justifyContent: 'end', width: '100%', marginTop: 20, paddingRight: 20, marginBottom: -10 }}>
                    <VectorHeart />
                </div>
                <div style={{ padding: "0 24px 24px 24px" }}>
                    <WidgetHeader>
                        <span role="left">
                            <div>
                                <Avatar src={userAvatar} size={86}>{lastName ? lastName.charAt(0).trim().toUpperCase() : 'A'}</Avatar>
                            </div>
                            <span>
                                <Typography.Title style={{ fontSize: 24, fontWeight: '700', fontFamily: "Inter" }}>{lastName || firstName ? firstName + " " + lastName : "Ẩn danh"}</Typography.Title>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
                        <span style={{ height: 'auto', width: '45%', display: 'flex', alignItems: 'center' }}>
                            ...
                            <div style={{ fontSize: 16, fontWeight: 600, fontFamily: "Inter", color: '#7A7A7A' }}>Location</div>
                        </span>
                        <span style={{ fontSize: 16, fontWeight: 600, fontFamily: "Inter" }}>Ho chi minh ctiy</span>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', marginBottom: 16 }}>
                        <span style={{ height: 'auto', width: '45%', display: 'flex', alignItems: 'center' }}>
