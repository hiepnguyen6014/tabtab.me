...
import { userSavePost, userDeleteSavePost } from '@root/src/core/services';
...
export const ButtonSave = (props: Props) => {
  const { t, showIcon = true, idPost, bordered = "true",IconDefault, IconActive, bg } = props;
  ...
  return (
    <>
      <ButtonSaveWrap bordered={bordered} onClick={onSave} bg={bg}>
        {showIcon && isSave ? IconActive || <IconSaved  /> : IconDefault || <IconHeart type='empty' style={{transform: 'scale(.7)'}}/>}
