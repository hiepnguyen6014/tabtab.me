...
export const SeeAllImages = memo((props: Props) => {
  const { arrayImages, isVisible, setIsVisible } = props;
  const [imgCurrent, setImgCurrent] = useState(0);
  const [thumbsSwiper, setThumbsSwiper] = useState(null);
  const onSnapChange = (e: any) => {
    setImgCurrent(e.activeIndex + 1);
  };

  const DynamicAgentInfo = dynamic(
    () => import('../../shared/WidgetUserInfo/WidgetAgencyInfo').then(
      (mod: any) => mod.WidgetAgencyInfo
    )
  )
