import { IconCircle, IconTrash } from '@root/public/icons';
  ...
  return (
    <>
      <NotifyItemRow theme={isRead == 0} key={messageCustomerId}>
        <NotifyMainCol
          wrap={false}
          onClick={() => onRedirect(detailHref.href, detailHref.idPost)}
        >
          <NotifyIconCol>{ROUTES_NOTIFY[messageType].icon}</NotifyIconCol>
