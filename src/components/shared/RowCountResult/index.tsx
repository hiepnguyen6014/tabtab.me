    ...
    return (
      <Menu defaultSelectedKeys={['desc']}>
        {listSortPrice.map((item, key) => (
          <Menu.Item key={key}>
            <a onClick={() => handleSort(item.query)}>{item.label}</a>
          </Menu.Item>
        ))}
      </Menu>
    );
    ...
    <Row justify="space-between" align="middle">
      <Col>
        <Typography.Title level={5}>
          {props.t('common.preResult')} {props.count.toLocaleString()}{' '}
          {props.t('common.subResult')}
        </Typography.Title>
      </Col>
      <Col>
        <Dropdown trigger={['click']} overlay={overlayMenu}>
          <p style={{ cursor: 'pointer' }}>
            {router?.query['sx']
              ? listSortPrice.filter(
                  item => item['query'] == router?.query['sx'],
                )[0]?.label
              : props.t('filter.filterPrice')}{' '}
            <IconArrowDown />
          </p>
        </Dropdown>
      </Col>
