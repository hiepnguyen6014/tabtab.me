...
import { IconFly, IconImage, IconMap, IconVirtual } from '@root/public/icons';
...
const LayoutDetail = (props: Props) => {
    ...
    getDetailPost(idPost ? Number(idPost) : 0)
      .then((res: MPostDetail) => {
        if (res && mounted && window) {
          if (
            (res.realEstateId &&
              getPostTypeId(window.location.pathname).id == 1 &&
              res.agency == 1) ||
            (getPostTypeId(window.location.pathname).id == 2 &&
              res.agency == 0 &&
              res?.isHidden !== 1)
          ) {
            setDataDetail(res);
  ...
  const DynamicMap: any = dynamic(() =>
    import('./LayoutComponent/LayoutMap').then((mod: any) => mod.LayoutMap),
  );
